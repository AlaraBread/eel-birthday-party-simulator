...
@onready var save_on_set:bool = OS.get_name() == "Web"

## automatically read data from file on startup.
@export var load_on_ready:bool = true

@onready var data:Dictionary = default_data()
...
func set_value(key, value):
	...
	if(save_on_set):
		save_to_file()
	if(key == "money"):
		Global.money_changed.emit(value)
...
func reset():
	var old_audio = data["audio_volume"]
	var old_rebinds = data["rebinds"]
	data = default_data()
	data["audio_volume"] = old_audio
	data["rebinds"] = old_rebinds
	save_to_file()
